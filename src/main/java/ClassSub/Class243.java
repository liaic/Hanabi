...
public class Class243
{
    ...
    public Class243(final String s) throws Class186 {
        this(Class160.getResourceAsStream(s));
    }
    
    public Class243(final InputStream inputStream) throws Class186 {
        super();
        this.fontSize = 12;
        this.bold = false;
        this.italic = false;
        ...
        try {
            ...
            while (true) {
                ...
                final String[] array = (String[])trim.split("=", 2);
                ...
                if (trim2.equals("font.size")) {
                    ...
                    if (trim2.equals("effect.class")) {
                        try {
                            ...
                            throw new Class186("Unable to create effect instance: " + string, ex);
                        }
                    }
                    ...
                    final Class208 class208 = (Class208)this.effects.get(this.effects.size() - 1);
                    final List values = class208.getValues();
                    for (final Class208.Class4 class209 : values) {
                        if (class209.getName().equals(substring)) {
                            class209.setString(string);
                            break;
                        }
                    }
                    class208.setValues(values);
                }
            }
            bufferedReader.close();
        }
        catch (Exception ex2) {
            throw new Class186("Unable to load Hiero font file", ex2);
        }
    }
    ...
    public void save(final File file) throws IOException {
        ...
        printStream.println();
        for (final Class208 class208 : this.effects) {
            printStream.println("effect.class=" + class208.getClass().getName());
            for (final Class208.Class4 class209 : class208.getValues()) {
                printStream.println("effect." + class209.getName() + "=" + class209.getString());
            }
            printStream.println();
        }
        printStream.close();
