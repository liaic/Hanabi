...
public class Class99
{
    ...
    public Class99(final Minecraft mc, final ScaledResolution sr) {
        super();
        ...
        this.fileDir = mc.field_71412_D.getAbsolutePath() + "/" + "Hanabi";
        this.mc = mc;
        this.sr = sr;
        this.loadFile();
    }
    ...
    public void draw() {
        ...
        final StaffAnalyzer staffAnalyzer = (StaffAnalyzer)Hanabi.INSTANCE.moduleManager.getModule(StaffAnalyzer.class);
        ...
        this.animationY = (float)Class284.getAnimationState(this.animationY, this.rectYSize, (double)Math.max(10.0f, Math.abs(this.animationY - this.rectYSize) * 30.0f) * 0.3);
        this.drawRoundedRect(this.x, this.y + 10.0f, this.x + 80.0f, this.y + this.animationY, Class128.reAlpha(Class59.WHITE.c, 0.5f), Class128.reAlpha(Class59.WHITE.c, 0.5f));
        if (!staffAnalyzer.onlinemod.isEmpty()) {
            if (this.isExtended) {
                float y = this.y + 13.0f;
                for (final String s : staffAnalyzer.onlinemod) {
                    if (this.y + this.animationY > y) {
                        Hanabi.INSTANCE.fontManager.wqy16.drawString(s, this.x + 2.0f, y, Class59.GREY.c);
                    }
                ...
                for (final String s2 : staffAnalyzer.onlinemod) {
                    if (this.y + this.animationY > y2) {
                        Hanabi.INSTANCE.fontManager.wqy16.drawString(s2, this.x + 2.0f, y2, Class59.GREY.c);
                    }
            ...
            Hanabi.INSTANCE.fontManager.wqy16.drawString("当前没有管理员在线", this.x + 2.0f, this.y + 14.0f, Class59.GREY.c);
        }
        final int rgb = new Color(-14848033).brighter().getRGB();
        this.drawRoundedRect(this.x, this.y, this.x + 80.0f, this.y + 13.0f, rgb, rgb);
        Hanabi.INSTANCE.fontManager.wqy16.drawString("在线管理 (" + staffAnalyzer.onlinemod.size() + "/" + StaffAnalyzer.modlist.length + ")", this.x + 2.0f, this.y + 2.0f, Class59.WHITE.c);
        Hanabi.INSTANCE.fontManager.icon16.drawString(this.isExtended ? HanabiFonts.ICON_CLICKGUI_ARROW_DOWN : HanabiFonts.ICON_CLICKGUI_ARROW_UP, this.x + 70.0f, this.y + 2.0f, Class59.WHITE.c);
    }
